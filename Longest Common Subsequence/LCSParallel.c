...
**
**
** Assumptions : Input files don't contain more than 1000 lines
*/
...
//	int filepos = 0;
	...
	while( ( size = getdelim( &buff, &buffsize, ( int) delim, fp)) != -1){

//		rec->line[filepos] = filepos;
		rec->str[rec->size] = ( char*) malloc( size * sizeof( char) );

		// To remove leading " " in a sentence
		if(buff[0] == ' '){
			buff++;
		}
...
void main(int argc,char* argv[]){
	...
	#pragma omp parallel for private(fr1,fr2,i,j)
		for(int i = 0; i < N; i++){
			for(int j = i + 1; j < N; j++){
	...
	// To be included in parallel
	omp_set_num_threads(16);
	#pragma omp parallel for private(j,fr1,fr2)
	...
	// To be included in parallel
		for(i = 0; noFiles != 1; i++){
			#pragma omp parallel for private(fr1,fr2)
			for( j = 0; j <= noFiles/2; j++){
				fr1 = combine[j];
